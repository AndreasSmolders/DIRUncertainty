...
from Utils.conversion import toIntMaybe
...
class GaussianBlur(nn.Module):
    def __init__(
        ...
        self.kernel: torch.Tensor = self.create_gaussian_kernel(
            kernel_size, sigma, self.rescale
        )
        self.nbChannels = 1  # will be altered on first iteration
        self.padding = padding
        assert self.padding == 0  # we drop the support for same padding

    def isEqual(self, other) -> bool:
        if isinstance(other, GaussianBlur):
            return (
                self.kernel_size == other.kernel_size
                and torch.all(self.kernel == other.kernel)
                and self.rescaleTensor == other.rescaleTensor
            )
        raise NotImplementedError

    @staticmethod
    def create_gaussian_kernel(kernel_size, sigma, rescale) -> torch.Tensor:
